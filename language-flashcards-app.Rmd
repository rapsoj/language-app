...
getCurrentWord <- function() {
  current_index <- flashcards()[card_num() - 1]
  list(
    points = ifelse(is.na(table()[current_index,]$Points[1]), 0,
                   as.numeric(table()[current_index,]$Points[1])),
    id = table()[current_index,]$id[1]
  )
}
...
observeEvent(input$correct | input$incorrect, {
  ...
  range_write(rv$valid_url, sheet = input$language,
              data = data.frame(x = Sys.Date()) %>% row_to_names(1),
              range = paste0("F", word$id), reformat = F)
})
...
observeEvent(input$correct, {
  req(rv$valid_url)
  word <- getCurrentWord()
  new_points <- word$points + 1
  
  # Update points
  range_write(rv$valid_url, sheet = input$language,
              data = data.frame(setNames(
                data.frame(matrix(0, nrow = 0, ncol = 1)),
                as.character(new_points)),
                check.names = F),
              range = paste0("G", word$id), reformat = F)
  
  # Update status
  if(new_points >= 4) {
    range_write(rv$valid_url, sheet = input$language,
                data = data.frame(Learned = character()),
                range = paste0("D", word$id), reformat = F)
  } else {
    range_write(rv$valid_url, sheet = input$language,
...
# Remove a point if answer is incorrect
observeEvent(input$incorrect, {
  req(rv$valid_url)
  word <- getCurrentWord()
  new_points <- max(0, word$points - 1)
